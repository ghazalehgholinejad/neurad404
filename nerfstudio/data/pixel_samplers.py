...
class PixelSampler:
    ...
    def collate_image_dataset_batch_list(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        if "depth_image" in batch:
            # print("1111111212138238913223923972382372183283792378238237283283727837278323782378238723871287312")
...
class ScaledPatchSampler(PixelSampler):
    ...
    def collate_image_dataset_batch(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        num_images, image_height, image_width, _ = batch["image"].shape
        image = batch["image"]
        depth_image = batch.get("depth_image", None)
        # print("depthhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
        ...
        _, depth_patches = self._patches_from_centers(depth_image, patch_center_indices, rgb_size, device)
    ...
    def collate_image_dataset_batch_list(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        for img_idx, patches_in_img in zip(img_indices, img_counts):
            ...
            ## EDIT
            depth = batch["depth_image"][img_idx].unsqueeze(0)
            ...
            ray_indices, img_patches = self._patches_from_centers(image, patch_center_indices, rgb_size, device)
            ray_indices, depth_patches = self._patches_from_centers(depth, patch_center_indices, rgb_size, device)
            all_img_patches.append(img_patches)
            all_depth_patches.append(depth_patches)
        ...
        depth_patches = torch.cat(all_depth_patches, dim=0)
        ray_indices[:, 0] = batch["image_idx"][ray_indices[:, 0]]  # use the "global" image idx

        collated_batch = {"indices": ray_indices, "image": img_patches, "depth": depth_patches}
        if keep_full_image:
            collated_batch["full_image"] = batch["image"]
        return collated_batch
    ...
    def _check_extra_keys(self, batch):
        ...
        extra_keys = set(batch.keys()) - {"image", "image_idx", "depth_image"}
        if extra_keys:
