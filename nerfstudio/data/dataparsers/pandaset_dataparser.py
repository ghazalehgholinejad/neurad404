...
from nerfstudio.data.dataparsers.ad_depth_dataparser import ADDepthDataParser
...
@dataclass
class PandaSet(ADDepthDataParser):
    ...
    def _get_depth_filenames(self, image_filenames: List[Path]) -> List[Path]:
        return [Path(str(image_filename).replace("camera", "depth",1).replace(".jpg", ".png")) for image_filename in image_filenames]
